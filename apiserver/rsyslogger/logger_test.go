...
func (s *loggerSuite) setRSysLoggingConfig(c *gc.C, rsyslogURL, rsyslogCACert string) {
	err := s.State.UpdateModelConfig(map[string]interface{}{"rsyslog-url": rsyslogURL, "rsyslog-ca-cert": rsyslogCACert}, nil, nil)
	c.Assert(err, jc.ErrorIsNil)
...
func (s *loggerSuite) TestWatchLoggingConfig(c *gc.C) {
	...
	s.setRSysLoggingConfig(c, "https://localhost:1234", coretesting.CACert)
...
func (s *loggerSuite) TestLoggingConfigForNoone(c *gc.C) {
	...
	results := s.logger.RsyslogURLConfig(params.Entities{})
	c.Assert(results.Results, gc.HasLen, 0)

	results = s.logger.RsyslogCACertConfig(params.Entities{})
	c.Assert(results.Results, gc.HasLen, 0)
}
...
func (s *loggerSuite) TestLoggingConfigRefusesWrongAgent(c *gc.C) {
	...
	results := s.logger.RsyslogURLConfig(args)
	c.Assert(results.Results, gc.HasLen, 1)
	result := results.Results[0]
	c.Assert(result.Error, gc.DeepEquals, apiservertesting.ErrUnauthorized)

	results = s.logger.RsyslogCACertConfig(args)
	c.Assert(results.Results, gc.HasLen, 1)
	result = results.Results[0]
	c.Assert(result.Error, gc.DeepEquals, apiservertesting.ErrUnauthorized)
}
...
func (s *loggerSuite) TestLoggingConfigForAgent(c *gc.C) {
	...
	c.Assert(result.Result, gc.Equals, "https://localhost:1234")

	results = s.logger.RsyslogCACertConfig(args)
	c.Assert(results.Results, gc.HasLen, 1)
	result = results.Results[0]
	c.Assert(result.Error, gc.IsNil)
	c.Assert(result.Result, gc.Equals, coretesting.CACert)
}

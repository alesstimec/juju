...
func (s *debugLogDBIntSuite) TestJSONFormat(c *gc.C) {
	...
	stop := make(chan struct{})
	done := s.runRequest(&debugLogParams{jsonFormat: true}, stop)

	s.assertOutput(c, []string{
		"ok", // sendOk() call needs to happen first.
		`{"Time":"2015-06-19T15:34:37Z","Entity":"machine-99","Module":"some.where","Location":"code.go:42","Level":3,"Message":"stuff happened","ModelUUID":""}`,
		`{"Time":"2015-06-19T15:36:40Z","Entity":"unit-foo-2","Module":"else.where","Location":"go.go:22","Level":5,"Message":"whoops","ModelUUID":""}`,
	})

...
func (s *logReaderSuite) TestLoggingConfigErro(c *gc.C) {
	...
	apiCaller := basetesting.APICallerFunc(
		func(objType string,
			...
			called = true
			switch request {
			case "RsyslogURLConfig":
				return errors.New("permission denied")
			case "RsyslogCACertConfig":
				return errors.New("permission denied")
			default:
	...
	url, err := api.RsyslogURLConfig(tag)
	c.Assert(err, gc.ErrorMatches, "permission denied")
	c.Assert(url, gc.Equals, "")
	c.Assert(called, jc.IsTrue)
	called = false

	caCert, err := api.RsyslogCACertConfig(tag)
	c.Assert(err, gc.ErrorMatches, "permission denied")
	c.Assert(caCert, gc.Equals, "")
	c.Assert(called, jc.IsTrue)
}
...
func (s *logReaderSuite) TestLoggingConfig(c *gc.C) {
	...
	apiCaller := basetesting.APICallerFunc(
		func(objType string,
			...
			switch request {
			case "RsyslogURLConfig":
				result, ok := response.(*params.StringResults)
				c.Assert(ok, jc.IsTrue)
				result.Results = []params.StringResult{{Result: "https://localhost:1234"}}
			case "RsyslogCACertConfig":
				result, ok := response.(*params.StringResults)
				c.Assert(ok, jc.IsTrue)
				result.Results = []params.StringResult{{Result: coretesting.CACert}}
			default:
				c.Fatalf("unknown request: %v", request)
			}
			return nil
	...
	c.Assert(url, gc.Equals, "https://localhost:1234")
	c.Assert(called, jc.IsTrue)
	called = false

	caCert, err := api.RsyslogCACertConfig(tag)
	c.Assert(err, jc.ErrorIsNil)
	c.Assert(caCert, gc.Equals, coretesting.CACert)
	c.Assert(called, jc.IsTrue)
}

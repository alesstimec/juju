...
func (st *State) AddRelation(eps ...Endpoint) (r *Relation, err error) {
	...
	buildTxn := func(attempt int) ([]txn.Op, error) {
		...
		for _, ep := range eps {
			...
			existingRelationCount, err := enforceMaxRelationLimit(app, ep)
			if err != nil {
				return nil, errors.Trace(err)
			}
...
// charm metadata. It also returns the number of existing relations, if a limit
// is to be imposed, or -1 in the case where there is no limit.
func enforceMaxRelationLimit(app ApplicationEntity, ep Endpoint) (int, error) {
	// No limit defined
	...
	if establishedCount+1 > ep.Relation.Limit {
		return 0, errors.QuotaLimitExceededf("establishing a new relation for %s:%s would exceed its maximum relation limit of %d", ep.ApplicationName, ep.Relation.Name, ep.Relation.Limit)
	}

	return establishedCount, nil
}
